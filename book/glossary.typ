...
#table(
  ...
  [derived state], [A stored structure that answers from current facts and owes its maintainer a correctness contract on every contributing write: an index, a rollup, a rolldown or an external read copy.],
  [retained history], [Information kept so a question can be answered after the event was undone: an event/ledger table, an audit trail, a temporal history or a snapshot. A current-state-only representation cannot reconstruct what it has destroyed.],
  [invalidation], [Removing or marking a cache entry stale. It orders nothing about a read or a fill already in flight.],
  [fill lease], [A bounded ownership period that suppresses duplicate concurrent fills while it is valid and honoured. It is a load control, not a freshness proof.],
  [fencing token], [A monotonically increasing token checked at a protected sink, which rejects an operation whose token is older than one the sink has already accepted. It orders operations at the sink; it does not itself consult the source.],
  [source-generation validation], [Before publishing, checking that the authoritative current generation still equals the generation the reader observed, and refusing the fill if it moved. This closes the stale-fill race.],
  [cache CAS], [A conditional modification of cache state against an explicitly stated expected value or generation. It is not source-generation validation.],
  [hit validation], [Verifying a cached value against authoritative or version state at the hit boundary before serving it.],
  [transactional outbox], [Writing a change in the same transaction as the mutation so it can be published durably. It makes a committed change observable; it does not by itself make every cache hit current.],
  [TTL / early expiry], [A bound on how long an entry is eligible to serve, plus probabilistic early refill before many entries reach one deadline. It controls eligibility and load, not currentness, unless the declared contract is itself bounded by it.],
  [publication fence], [Refusing a fill whose source moved after the reader took its snapshot. This, not invalidation, is what closes the stale-fill race; source-generation validation is its strongest named form.],
