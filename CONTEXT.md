...
- **PHASE 3B RUNNING — do not start databases (from 2026-09-20, LOW, Claude Sonnet 5; resumed 2026-09-21 by deepseek-flash).** AM-04 passed (`5e3a3f8`, tag `study-02/v2.2-ledger-attribution`); calibration done (`am03-cal-small/SUMMARY.md`): study 02 reports at 10 s, study 03 at 5 s, race B2 = 128, estimate ~8 h. Runs go one at a time: 3b-1 (study 02 reports matrix), 3b-2 and 3b-3 (P3 → X1 race pair at 32 and 128 buyers), 3b-4 (study 03 reports matrix). Each holds the benchmark lock while it runs.
  - **Resumption note, 2026-09-21.** The 2026-09-20 session ended after 3b-1 finished (23:50:01Z → 02:38:52Z, 45 cells, 0 failed) but before committing its output; the results and report were left untracked in `.worktrees/recency-reports`, whose `.git` link had been written with a Windows path and needed `git worktree repair`. Both are dealt with: the link is repaired (and made *relative*, so WSL git and Git Bash's git.exe both resolve it — the previous form worked for only one of them), the CRLF phantom edits the checkout had accumulated were normalized from its own LF index, and 3b-1 is committed as `b0d9b69` (digest `160bd80c49bbd892`). The task's own model-role map (HIGH = Claude Opus 5, LOW = Claude Sonnet 5) is not the session that resumed it: `deepseek-flash` is executing AM-03.12/AM-03.13 and signing the analyses as a new analyst, and the handoff's HIGH validation of phase 3b remains open.
  - **Runs, one at a time, under the lock.** 3b-1 committed (`b0d9b69`, digest `160bd80c49bbd892`). A wiring dev check after the environment changes is `dad86d0` (X1, `pg-single`, `tiny`, 46/46). 3b-2 committed (`a04fd20`, digest `4a02ec8fb0991166`): 6 cells, and **one partial cell** — `yb-cluster3/x1_cas_ledger` stopped in race trial 3 at the 10 000-seat tier with a YugabyteDB RPC timeout (SQLSTATE XX000), after gate 48/48, every ledger audit consistent, both post-write report checks ok, and 16 of its 24 race/churn tiers recorded. AM-04.6 makes that "log and continue", not an escalation; the analysis reports the cell as partial. 3b-3 is running now (`20260921T205212Z`, X1 first, 128 buyers), 3b-4 next.
  - **Environment, and why runs go through Git Bash.** From WSL the branch's `infra/lib.sh` had no podman client, so `infra/lib.sh` now carries main's resolver (`2e31fa4`, 58 additive lines, no study code) and reports are committed. The measured runs themselves still execute under Git Bash, exactly as 3b-1 and the AM-04 dev checks did, where `cygpath` makes `hostpath()` the Windows form `podman.exe` needs.
