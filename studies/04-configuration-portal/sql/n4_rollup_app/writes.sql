...
-- name: w_recompute_rollup
-- params: installed_product_id
-- The application's half of the rollup contract, issued inside the same
-- transaction as the change it describes (n4) or after commit (x2, the control).
-- It is the same SQL for both: what differs is the transaction boundary, which
-- is the decision under test.
UPDATE installed_product ip SET
  config_count = (SELECT count(*) FROM config_entry c WHERE c.installed_product_id = $1),
  content_hash = COALESCE((SELECT md5(string_agg(c.key || '=' || c.value, ',' ORDER BY c.key))
                           FROM config_entry c WHERE c.installed_product_id = $1), ''),
  updated_at   = now()
WHERE ip.id = $1;
