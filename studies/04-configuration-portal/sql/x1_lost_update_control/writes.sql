...
-- name: wx1_read_value
-- params: installed_product_id, key
-- Identical to c2_pessimistic_lock's read. The difference between the two designs
-- is the missing wc2_lock_installed_product below, and nothing else.
SELECT value, version FROM config_entry WHERE installed_product_id = $1 AND key = $2;

-- name: wx1_write_value
-- params: installed_product_id, key, value
-- No `AND version = $4`. Two writers that both read version 7 both write version
-- 8, and one of the two increments is gone.
UPDATE config_entry
