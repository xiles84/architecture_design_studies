...
-- name: a_version_sum
-- params: installed_product_id
-- INV-3, from the design's own side: the sum of the increments actually applied.
-- Compared against the sum the harness believes it published. A gap is a lost
-- update, and it is the *only* shape a lost update can take here.
SELECT sum(version) AS version_sum FROM config_entry WHERE installed_product_id = $1;
