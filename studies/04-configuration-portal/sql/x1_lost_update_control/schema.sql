...
-- NEGATIVE CONTROL. This design reads the current value and writes back a value
-- derived from it, with no version guard and no lock -- the classic
-- read-modify-write. Two concurrent updates of the same key can both be
-- acknowledged while only one survives (INV-3), and the harness's ledger will
-- show an acknowledged publication that the database does not hold.
--
-- It is measured like every other design and its speed is never reported as a
-- plain number: a design that loses updates quickly has not been fast.
