...
-- name: wc1_read_version
-- params: installed_product_id, key
SELECT value, version FROM config_entry WHERE installed_product_id = $1 AND key = $2;

-- name: wc1_update_guarded
-- params: installed_product_id, key, value, expected_version
-- The guard. A zero-row result is not an error: it is this design's arbitration
-- signal, and the retry is counted (INV-13).
UPDATE config_entry
