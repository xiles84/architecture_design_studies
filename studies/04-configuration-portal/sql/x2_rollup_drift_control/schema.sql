...
-- NEGATIVE CONTROL. Every table, index, statement and transaction body here is
-- byte-identical to n4_rollup_app. The single difference is where the harness
-- calls w_recompute_rollup: x2 issues it in a second transaction, after the
-- publication has already committed and been acknowledged. Under concurrency
-- that leaves a window in which the stored rollup is stale -- or, if another
-- writer's recompute wins, permanently wrong. The audit must catch it; if it
-- does not, the audit does not work and no design's rollup may be trusted.
