...
-- name: wc2_lock_installed_product
-- params: installed_product_id
-- Lock before the competing read. The row lock is the entire difference from c1;
-- it is also what the harness waits on, so time spent here is contention, not
-- latency, and it is reported separately.
