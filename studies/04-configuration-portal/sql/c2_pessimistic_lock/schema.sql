...
-- This design takes a row lock on the installed product before reading anything,
-- so the competing writer waits instead of being rejected. It uses the same
-- tables as c1, guard column included: the pair isolates arbitration, and giving
-- the two designs different schemas would move a second mechanism with it.
