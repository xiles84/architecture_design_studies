...
-- name: w02_add_key
-- params: installed_product_id, key, value
INSERT INTO config_entry (installed_product_id, product_definition_id, environment_id, business_unit_id, key, value)
SELECT $1, ip.product_definition_id, ip.environment_id, ip.business_unit_id, $2, $3
...
INSERT INTO config_entry (installed_product_id, product_definition_id, environment_id, business_unit_id, key, value)
SELECT $1, ip.product_definition_id, ip.environment_id, ip.business_unit_id, t.k, t.v
FROM unnest($2::text[], $3::text[]) AS t(k, v)
CROSS JOIN installed_product ip WHERE ip.id = $1
ON CONFLICT (installed_product_id, key)
DO UPDATE SET value = EXCLUDED.value, version = config_entry.version + 1, updated_at = now();
...
-- name: w_load_entries
INSERT INTO config_entry (installed_product_id, product_definition_id, environment_id, business_unit_id, key, value)
SELECT le.installed_product_id, ip.product_definition_id, ip.environment_id, ip.business_unit_id, le.key, le.value
FROM load_entry le JOIN installed_product ip ON ip.id = le.installed_product_id;
