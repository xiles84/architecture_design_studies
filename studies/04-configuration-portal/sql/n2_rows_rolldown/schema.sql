...
-- Rolldown: the parent's identity, copied onto every configuration row. It
-- exists so that the cross-installation search (r05) never has to join back to
-- installed_product. The copied columns are nullable rather than NOT NULL
-- because the loader fills them after the table exists, and a NOT NULL default
-- would have to be a value no product has.
ALTER TABLE config_entry ADD COLUMN product_definition_id int REFERENCES product_definition(id);
ALTER TABLE config_entry ADD COLUMN environment_id        int REFERENCES environment(id);
ALTER TABLE config_entry ADD COLUMN business_unit_id      int REFERENCES business_unit(id);
