-- Query catalogue for D1 (normalized-minimal).
--
-- Harness annotations:
--   -- name:   <query id>    identifier used in results and reports
--   -- params: <kinds>       comma-separated; the harness binds a fresh random
--                            value per execution. Kinds: charity_id, person_id,
--                            donation_id, none.
--
-- D1 and D2 share byte-identical SQL. Only the index set differs, which makes
-- that pair a clean controlled experiment on indexing alone.
