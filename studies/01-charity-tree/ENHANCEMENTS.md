...
| Mechanisms | D11 copies the key with D2 indexes/reads; D12 adds the date index; D13 adds recency rewrites; D14 adds a plain charity index and sum rewrite; D15 replaces that plain index with INCLUDE(amount_cents); D3 adds the ranking rewrites. Register controlled pairs, with SQL differences explicit |
