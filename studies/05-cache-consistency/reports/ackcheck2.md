# Study 05 — external cache throughput and consistency — run `ackcheck2`

Generated 2026-09-21T16:33:59Z from 3 result file(s). This report contains measurements only; conclusions live in
...
| Benchmark image | `localhost/cachebench:1` (`77e3488edf42d814ac5`) |
| Resource framing | `db-only` |
| **Inputs digest** | `66b7acb3ea6ae9a0` |
| Cells | 3 |
...
- Cells reported: 3; cells that failed: 2.
  - failed: `pg-single/legacy-na-memory-aside-strict-coord` (phase ackcheck: ACK CHECK FAILED: 2 of 310 acknowledgements were not visible to a second session immediately afterwards (concurrent: acknowledged donor 70 and a…)
  - failed: `pg-single/ref-normalized-indexed` (phase ackcheck: ACK CHECK FAILED: 4 of 310 acknowledgements were not visible to a second session immediately afterwards (concurrent: acknowledged donor 3 and a …)
...
No signed analysis exists yet for inputs digest `66b7acb3ea6ae9a0`.
...
| pg-single | `o:opt:db` | 7 | 0 | 0 | db-only | 8 |
