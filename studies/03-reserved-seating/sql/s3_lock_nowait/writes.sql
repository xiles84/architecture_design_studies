-- S3 — lock-nowait: every statement the hold, checkout, sweeper, refund
-- and publishing flows issue.
--
-- Hold, one READ COMMITTED transaction:
--   w_lock_seats      -> lock the block's rows in seat order, NOWAIT; any seat not free or locked:
--                        ROLLBACK, re-read the seat map, choose again
--   w_hold_seats      -> S1's statement; with the rows locked it must take them all
--
...
-- The design: lock first, look, then write. ORDER BY is applied before the rows
-- are locked, so overlapping blocks lock their shared seats in the same order.
-- NOWAIT: a seat another buyer is taking fails at once (55P03) -- choose again.
SELECT seat_id,
...
-- S1's conditional update. The rows are locked and were seen free, so anything
-- less than every seat is an invariant violation, not a lost race.
UPDATE event_seat
   SET status = 'held',
       hold_id = $1,
       customer_id = $2,
