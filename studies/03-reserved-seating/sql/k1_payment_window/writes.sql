-- K1 — payment-window: every statement the hold, checkout, sweeper, refund
...
--   w_check_hold
--
-- Start of checkout, one transaction:
--   w_begin_checkout  -> fewer rows = the hold expired before paying: ROLLBACK,
--                        tell the buyer now, nothing was charged
--
...
-- S1's hold. A stolen expired hold also loses its checkout mark.
UPDATE event_seat
   SET status = 'held',
       hold_id = $1,
       customer_id = $2,
...
-- The design: a valid hold is extended to cover the payment, once. The caller
-- rolls back unless every seat was extended.
UPDATE event_seat
...
-- params: event_id, section_no, hold_id, seat_ids
-- S1's checked confirmation, against the possibly extended expiry.
UPDATE event_seat
   SET status = 'sold',
       sold_at = now()
 WHERE event_id = $1
