-- E0 — app-clock-expiry (NEGATIVE CONTROL): every statement the hold, checkout, sweeper, refund
...
--
-- $app_now is the clock of the simulated application node issuing the statement.
-- Every statement that grants, extends, releases or sells still returns the
-- database's now() AS db_now: the ledger judges theft and late sales by it.

-- name: w_hold_seats
-- params: hold_id, customer_id, hold_ms, event_id, section_no, seat_ids, app_now
-- S1's hold, with the expiry written and judged by the application node's clock.
-- A node running fast sees valid holds as expired and takes their seats.
UPDATE event_seat
   SET status = 'held',
       hold_id = $1,
       customer_id = $2,
       hold_expires_at = $7::TIMESTAMPTZ + make_interval(secs => $3::FLOAT8 / 1000.0)
 WHERE event_id = $4
   AND section_no = $5
   AND seat_id = ANY ($6::INT[])
   AND (status = 'available' OR (status = 'held' AND hold_expires_at <= $7::TIMESTAMPTZ))
RETURNING seat_id, now() AS db_now, clock_timestamp() AS db_clock, hold_expires_at AS expires_at;

-- name: w_check_hold
-- params: event_id, section_no, hold_id, seat_ids, app_now
...
SELECT COUNT(*) = cardinality($4::INT[]) AS valid
 ...
 WHERE event_id = $1
   ...
   AND hold_id = $3
   AND status = 'held'
   AND hold_expires_at > $5::TIMESTAMPTZ;

-- name: w_confirm_seats
-- params: event_id, section_no, hold_id, seat_ids, app_now
-- Checked confirmation -- but "unexpired" is judged by the confirming application
-- node's clock ($app_now), not the database's.
UPDATE event_seat
   SET status = 'sold',
       sold_at = now()
 WHERE event_id = $1
...
-- params: event_id, section_no, seat_ids, app_now
...
UPDATE event_seat
 ...
 WHERE event_id = $1
   ...
   AND status = 'held'
   AND hold_expires_at <= $4::TIMESTAMPTZ
RETURNING seat_id, now() AS db_now, clock_timestamp() AS db_clock;
