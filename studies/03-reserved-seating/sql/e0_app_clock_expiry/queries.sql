-- E0 — app-clock-expiry (NEGATIVE CONTROL): the read catalogue.
--
-- Names and result shapes are identical in every design; verification compares
-- them with Go-computed truth. Expiry is judged at read time -- by the clock of
-- the application node issuing the query ($app_now), not the database's.

-- name: q01_section_map
-- params: event_id, section_no, app_now
...
SELECT section_no,
       COUNT(*) FILTER (WHERE status = 'available'
                           OR (status = 'held' AND hold_expires_at <= $2::TIMESTAMPTZ))::BIGINT AS available
  FROM event_seat
 WHERE event_id = $1
 GROUP BY section_no
 ORDER BY section_no;
...
SELECT COUNT(*)::BIGINT AS available
  FROM event_seat
 WHERE event_id = $1
   AND (status = 'available' OR (status = 'held' AND hold_expires_at <= $2::TIMESTAMPTZ));

-- name: q04_hold_seats
-- params: event_id, section_no, hold_id, app_now
