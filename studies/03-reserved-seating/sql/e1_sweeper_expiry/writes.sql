-- E1 — sweeper-expiry: every statement the hold, checkout, sweeper, refund
...
-- The design. Takes only seats whose status says available: an expired hold the
-- sweeper has not released yet is NOT free here. The caller rolls back unless
...
-- touching. Here the sweeper is what makes expiry take effect: until it runs, an
-- expired hold keeps its seat.
