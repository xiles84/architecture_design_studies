-- S2 — lock-then-update: every statement the hold, checkout, sweeper, refund
-- and publishing flows issue.
--
-- Hold, one READ COMMITTED transaction:
--   w_lock_seats      -> lock the block's rows in seat order; any seat not free:
...
-- A buyer reaching for a seat another buyer is taking waits here.
SELECT seat_id,
