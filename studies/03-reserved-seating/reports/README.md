...
podman run --rm -v "$PWD/../results:/results" -v "$PWD:/reports" \
  localhost/seatbench:1 -cmd report -results /results/<run-id> -report-out /reports/<run-id>.md
```
...
`run/03-reserved-seating/<run-id>` when the run was started with `--tag`. That identifies the
