...
func runValidate(args []string, out, errOut io.Writer) int {
	...
	claims := fs.String("claims", "book/evidence/claims.json", "registry file (repo-relative)")
	schemaPath := fs.String("schema", "book/evidence/claims.schema.json", "JSON Schema file (repo-relative)")
	...
	if *jsonOut {
		...
		fmt.Fprintf(out, "evidence: %d claims, 0 errors, %d book inputs\n", len(doc.Claims), len(doc.BookInputs()))
	} else {
		for _, e := range errs {
